...
`define ROM_WRITE_WAIT_CYCLES   5'd16


// ���ڸ��¹̼�ģ��
module sy_uart_debug(

    input wire clk,                // ʱ���ź�
    input wire rst,                // ��λ�ź�

    input wire debug_en_i,         // ģ��ʹ���ź�
    ...
    reg[4:0] write_wait_cnt;

    reg[15:0] crc_result;
    reg[3:0] crc_bit_index;
    reg[7:0] crc_byte_index;

    wire write_mem_done = (write_mem_byte_index0 > (need_to_rec_bytes - 2));
    wire write_mem_fire = (state == S_WRITE_MEM) && (write_wait_cnt == 5'h1) && !write_mem_done;
            ...
            case (state)
                ...
                S_WRITE_MEM: begin
                    if (write_mem_done) begin
                        ...
                        mem_we_o <= 1'b0;
                        state <= S_SEND_ACK;
                    end else if (write_wait_cnt == 5'h0) begin
                        write_wait_cnt <= write_wait_cnt + 1'b1;
                        mem_addr_o <= write_mem_addr;
                        mem_wdata_o <= write_mem_data;
                        mem_we_o <= 1'b1;
                    end else if (write_wait_cnt == 5'h1) begin
                        write_wait_cnt <= write_wait_cnt + 1'b1;
                        mem_addr_o <= write_mem_addr;
                        mem_wdata_o <= write_mem_data;
                        mem_we_o <= 1'b1;
                    end else if (write_wait_cnt == `ROM_WRITE_WAIT_CYCLES) begin
                        write_wait_cnt <= 5'h0;
                        mem_addr_o <= `UART_STATUS_REG;
                        mem_wdata_o <= 32'h0;
                        mem_we_o <= 1'b0;
    ...
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            ...
            case (state)
                ...
                S_WRITE_MEM: begin
                    if (write_mem_fire && write_mem_byte_index0 < (need_to_rec_bytes - 2)) begin

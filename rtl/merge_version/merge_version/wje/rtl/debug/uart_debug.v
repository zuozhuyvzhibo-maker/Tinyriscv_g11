...
module wje_uart_debug(
    ...
    input wire mem_busy_i          // NEW: wje_bridge busy backpressure for paced ROM writes
    ...
    localparam WRITE_MEM_IDLE_ADDR       = 32'hf0000000; // NEW: park on an unmapped address between ROM writes to avoid duplicate wje_bridge starts
    ...
    reg write_mem_busy_seen;       // NEW: records that wje_bridge has accepted the current ROM write
    ...
    wire write_mem_advance = (state == S_WRITE_MEM) &&
                             ...
                             (mem_busy_i == 1'b0); // NEW: advance only after wje_bridge busy rises and then falls
            ...
            case (state)
                ...
                S_WRITE_MEM: begin
                    if (write_mem_byte_index0 == (need_to_rec_bytes + 2)) begin
                        mem_addr_o <= WRITE_MEM_IDLE_ADDR; // CHANGE: leave ROM/RAM wje_bridge path before ACK because req_o stays high in debug mode
                        ...
                        mem_addr_o <= WRITE_MEM_IDLE_ADDR; // NEW: one-cycle unmapped gap prevents wje_bridge from re-sampling the old ROM word
                        ...
                        if ((write_mem_busy_seen == 1'b0) && (mem_busy_i == 1'b1)) begin
                            write_mem_busy_seen <= 1'b1;  // NEW: wje_bridge has seen this word; hold it until busy returns low
                        end
                    end
                end
                S_SEND_ACK: begin
            ...
            case (state)
                ...
                S_WRITE_MEM: begin
                    if (write_mem_advance == 1'b1) begin
                        write_mem_addr <= write_mem_addr + 4; // CHANGE: advance address only after current wje_bridge write completes
                    end
                end
            endcase
        end
    ...
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            ...
            case (state)
                ...
                S_WRITE_MEM: begin
                    if (write_mem_advance == 1'b1) begin
                        write_mem_data <= {rx_data[write_mem_byte_index3], rx_data[write_mem_byte_index2], rx_data[write_mem_byte_index1], rx_data[write_mem_byte_index0]}; // CHANGE: prepare next word only after current write completes
                    end
                end
            endcase
        end
    ...
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            ...
            case (state)
                ...
                S_WRITE_MEM: begin
                    if (write_mem_advance == 1'b1) begin
                        write_mem_byte_index0 <= write_mem_byte_index0 + 4; // CHANGE: byte pointer advances once per completed wje_bridge write
                    end
                end
            endcase
        end
    ...
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            ...
            case (state)
                ...
                S_WRITE_MEM: begin
                    if (write_mem_advance == 1'b1) begin
                        write_mem_byte_index1 <= write_mem_byte_index1 + 4; // CHANGE: byte pointer advances once per completed wje_bridge write
                    end
                end
            endcase
        end
    ...
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            ...
            case (state)
                ...
                S_WRITE_MEM: begin
                    if (write_mem_advance == 1'b1) begin
                        write_mem_byte_index2 <= write_mem_byte_index2 + 4; // CHANGE: byte pointer advances once per completed wje_bridge write
                    end
                end
            endcase
        end
    ...
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            ...
            case (state)
                ...
                S_WRITE_MEM: begin
                    if (write_mem_advance == 1'b1) begin
                        write_mem_byte_index3 <= write_mem_byte_index3 + 4; // CHANGE: byte pointer advances once per completed wje_bridge write
                    end
                end
            endcase
        end

...
module wje_rib(
    ...
    // 原接口没有 s0_req_o，新增该信号用于通知 wje_bridge 当前 slave0 访问有效
    ...
    // 原接口没有 s1_req_o，新增该信号用于通知 wje_bridge 当前 slave1 访问有效
    ...
    // 因此最多支持16个从设备
    parameter [3:0]slave_0 = 4'b0000;       // 地址高 4 位为 0 时访问 slave0，当前对应 ROM/wje_bridge ROM 侧
    parameter [3:0]slave_1 = 4'b0001;       // 地址高 4 位为 1 时访问 slave1，当前对应 RAM/wje_bridge RAM 侧
    parameter [3:0]slave_2 = 4'b0010;       // 地址高 4 位为 2 时访问 slave2，原工程预留给 wje_timer
    ...
    parameter [3:0]slave_7 = 4'b0111;       // 地址高 4 位为 7 时访问 I2C，复用物理 slave2接口，即原来的wje_timer
    ...
    always @ (*) begin // 组合仲裁逻辑，根据请求向量实时选择一个 master
        if (req[3]) begin // master3 请求有效时优先级最高，通常对应 wje_uart_debug
    ...
    always @ (*) begin // 组合选路逻辑，根据授权 master 和地址高位选择目标 slave
        ...
        // 原默认值只清除 we/addr/data，没有 s*_req_o；外部 wje_bridge 需要明确的访问有效信号

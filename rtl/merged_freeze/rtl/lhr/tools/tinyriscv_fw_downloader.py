...
def build_packets(image_path: Path, image: bytes) -> list[bytes]:
    ...
    if len(image) > ROM_CAPACITY_BYTES:
        raise ValueError(
            f"image contains {len(image)} bytes, but the FPGA ROM holds "
            f"{ROM_CAPACITY_BYTES} bytes"
        )
    ...
    # The legacy receiver expects one trailing packet for exact multiples of
    # 32 bytes.  RTL word budgeting suppresses its padding, so a 1024-byte
    # image programs exactly 256 words and never accesses word 256.

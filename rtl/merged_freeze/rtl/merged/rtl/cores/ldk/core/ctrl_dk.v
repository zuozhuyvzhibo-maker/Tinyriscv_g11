
`include "ldk_defs.v"

// LDK-prefixed private RTL module for the four-core integration.
module ldk_ctrl_dk(
    input wire clk,
    input wire rst,


    input wire [`LDK_InstBus] inst_at_ex_i,

    // from ex
    input wire jump_flag_i,
    input wire [`LDK_InstAddrBus] jump_addr_i,
    input wire ext_mem_req_i,
    input wire ext_mem_we_i,
    input wire mem_no_ack_i,
    input wire hold_flag_ex_i,
    input wire ife_use_uart,
    input wire ext_inst_done,

    // from rib / bridge
    input wire hold_flag_rib_i,
    input wire if_ack_i,
    input wire mem_ack_i,


    // to pipeline registers
    output reg [`LDK_Hold_Flag_Bus] hold_flag_o,

    // to pc_reg
    output reg jump_flag_o,
    output reg [`LDK_InstAddrBus] jump_addr_o,

    // to bridge
    output wire if_req_o,
    output wire mem_req_o,
    output wire mem_we_o,


    output wire reg_we_gate_o,

    output wire ext_inst_start_o,


    output wire mem_rdata_use_latched_o,


    output wire [4:0] state_o
);

    // ========================================================================

    // ========================================================================
    localparam S_IF_REQ      = 4'd0;
    localparam S_IF_WAIT     = 4'd1;
    localparam S_LATCH_ID    = 4'd2;
    localparam S_EX          = 4'd3;
    localparam S_MEM_R_REQ   = 4'd4;
    localparam S_MEM_R_WAIT  = 4'd5;
    localparam S_MEM_W_REQ   = 4'd6;
    localparam S_MEM_W_WAIT  = 4'd7;
    localparam S_SID_REQ     = 4'd8;
    localparam S_SID_WAIT    = 4'd9;
    localparam S_RT_R_REQ    = 4'd10;
    localparam S_RT_R_WAIT   = 4'd11;
    localparam S_IFE_REQ     = 4'd12;
    localparam S_IFE_WAIT    = 4'd13;

    localparam S_DONE        = 4'd14;



    reg [4:0] state, next_state;


    reg                      jump_pending;
    reg [`LDK_InstAddrBus]       jump_addr_latched;

    // ========================================================================

    // ========================================================================
    wire [6:0] opcode = inst_at_ex_i[6:0];
    wire [2:0] funct3 = inst_at_ex_i[14:12];
    wire [6:0] funct7 = inst_at_ex_i[31:25];

    wire is_load_inst   = (opcode == `LDK_INST_TYPE_L);
    wire is_store_inst  = (opcode == `LDK_INST_TYPE_S);
    wire is_sw_inst     = is_store_inst && (funct3 == `LDK_INST_SW);
    wire is_sb_sh_inst  = is_store_inst && ((funct3 == `LDK_INST_SB) || (funct3 == `LDK_INST_SH));
    wire is_rT_inst     = ( opcode == `LDK_INST_EXTEND ) && ( funct3 == `LDK_INST_RT ) ;
    wire is_sID_inst    = ( opcode == `LDK_INST_EXTEND ) && ( funct3 == `LDK_INST_SID ) ;
    wire is_ife_inst    = ( opcode == `LDK_INST_EXTEND ) && ( funct3 == `LDK_INST_IFE ) ;
    wire is_sid_state   = ( state == S_SID_REQ ) || ( state == S_SID_WAIT ) ;
    wire is_ife_state   = ( state == S_IFE_REQ ) || ( state == S_IFE_WAIT ) ;


    // ========================================================================

    // ========================================================================
    always @(posedge clk) begin
        if (rst == `LDK_RstEnable) state <= S_IF_REQ;
        else                   state <= next_state;
    end

    // ========================================================================

    // ========================================================================
    always @(*) begin
        next_state = state;
            case (state)
                S_IF_REQ:     next_state = S_IF_WAIT;
                S_IF_WAIT:    if (if_ack_i == `LDK_RIB_ACK) next_state = S_LATCH_ID;
                S_LATCH_ID:   next_state = S_EX;
                S_EX: begin
                    if      (is_load_inst)    next_state = S_MEM_R_REQ;
                    else if (is_sw_inst)      next_state = S_MEM_W_REQ;
                    else if (is_sb_sh_inst)   next_state = S_MEM_R_REQ;
                    else if (is_rT_inst)      next_state = S_RT_R_REQ;
                    else if (is_sID_inst)     next_state = S_SID_REQ;
                    else if (is_ife_inst)     next_state = S_IFE_REQ;
                    else                      next_state = S_DONE;
                end
                S_MEM_R_REQ:  begin
                                  if (mem_no_ack_i) begin
                                      next_state = S_DONE;
                                  end
                                  else begin
                                      next_state = S_MEM_R_WAIT;
                                  end
                              end
                S_MEM_R_WAIT: if (mem_ack_i == `LDK_RIB_ACK) begin
                                  if (is_sb_sh_inst) next_state = S_MEM_W_REQ;
                                  else               next_state = S_DONE;
                              end
                S_MEM_W_REQ:  next_state = mem_no_ack_i ? S_DONE : S_MEM_W_WAIT;
                S_MEM_W_WAIT: if (mem_ack_i == `LDK_RIB_ACK) next_state = S_DONE;
                S_RT_R_REQ:   next_state = S_RT_R_WAIT;
                S_RT_R_WAIT:  if (ext_inst_done == `LDK_EXT_INST_DONE) next_state = S_DONE;
                S_SID_REQ:    next_state = S_SID_WAIT;
                S_SID_WAIT:   if (ext_inst_done == `LDK_EXT_INST_DONE) next_state = S_DONE;
                S_IFE_REQ:    if (ife_use_uart) next_state = S_IFE_WAIT; else next_state = S_DONE ;
                S_IFE_WAIT:   if (ext_inst_done == `LDK_EXT_INST_DONE) next_state = S_DONE;
                S_DONE:       next_state = S_IF_REQ;
                default:      next_state = S_IF_REQ;
            endcase
        end

    // ========================================================================

    // ========================================================================
    always @(*) begin
        case (state)

            S_IF_WAIT:   hold_flag_o = (if_ack_i == `LDK_RIB_ACK) ? `LDK_Hold_Pc : `LDK_Hold_Id;
            S_LATCH_ID:  hold_flag_o = `LDK_Hold_If;
            S_DONE:      hold_flag_o = `LDK_Hold_None;
            default:     hold_flag_o = `LDK_Hold_Id;
        endcase
    end

    // ========================================================================

    // ========================================================================
    always @(posedge clk) begin
        if (rst == `LDK_RstEnable) begin
            jump_pending      <= 1'b0;
            jump_addr_latched <= `LDK_ZeroWord;
        end else if ((state == S_EX) && (jump_flag_i == `LDK_JumpEnable)) begin
            jump_pending      <= 1'b1;
            jump_addr_latched <= jump_addr_i;
        end else if (state == S_DONE) begin
            jump_pending      <= 1'b0;
            jump_addr_latched <= `LDK_ZeroWord;
        end
    end

    // ========================================================================

    // ========================================================================
    always @(*) begin
        if ((state == S_DONE) && jump_pending) begin
            jump_flag_o = `LDK_JumpEnable;
            jump_addr_o = jump_addr_latched;
        end else begin
            jump_flag_o = `LDK_JumpDisable;
            jump_addr_o = `LDK_ZeroWord;
        end
    end

    // ========================================================================

    // ========================================================================


    assign if_req_o = (state == S_IF_REQ) ;


    assign mem_req_o = (state == S_MEM_R_REQ) || ( state == S_MEM_R_WAIT )
                        || (state == S_MEM_W_REQ) || ( state == S_MEM_W_WAIT )
                        || ( state == S_RT_R_REQ ) || ( state == S_RT_R_WAIT )
                        || ( (is_sid_state || is_ife_state) && ext_mem_req_i ) ;


    assign mem_we_o = (state == S_MEM_W_REQ)
                        || ( state == S_RT_R_REQ )
                        || ( (is_sid_state || is_ife_state) && ext_mem_we_i ) ;

    assign ext_inst_start_o = (( state == S_SID_REQ ) && is_sID_inst)
                           || (( state == S_IFE_REQ ) && is_ife_inst && ife_use_uart) ;


    assign mem_rdata_use_latched_o = is_sb_sh_inst
                                  && ((state == S_MEM_W_REQ));





    assign reg_we_gate_o = ((state == S_EX) && !is_load_inst && !is_store_inst && !is_sID_inst && !is_rT_inst && !is_ife_inst)
                        || ((state == S_MEM_R_WAIT) && (mem_ack_i == `LDK_RIB_ACK) && is_load_inst)
                        || ((state == S_MEM_R_REQ)  && mem_no_ack_i && is_load_inst)
                        || ((state == S_RT_R_WAIT)  && (ext_inst_done == `LDK_EXT_INST_DONE) && is_rT_inst)
                        || ((state == S_IFE_REQ)    && !ife_use_uart && is_ife_inst)
                        || ((state == S_IFE_WAIT)   && (ext_inst_done == `LDK_EXT_INST_DONE) && is_ife_inst) ;


    assign state_o = state;



endmodule

...
module lhr_i2c(
    ...
    always @ (posedge clk) begin
        if (rst == `LHR_RstEnable) begin
            slave_addr <= 7'h48;
            output_data <= 32'h0;
        end else if (we_i == `LHR_WriteEnable) begin
            case (addr_i[23:16])
    ...
    always @ (posedge clk) begin
        if (rst == `LHR_RstEnable) begin
            ...
            // Do not preload a plausible sensor value. A temperature value
            // becomes valid only after a real I2C read transaction.
            input_data <= 16'h0000;
            data_valid_o <= 1'b0;
            idle_cnt <= 20'h0;
            scl_reg <= 1'b1;
            ...
            case (state)
                ...
                S_READ_HIGH: begin
                    ...
                    if (bit_cnt == 3'h0) begin
                        if (step == 2'h2) begin
                            sample_data[15:8] <= {read_shift[7:1], sda_in};
                            step <= 2'h3;
                            state <= S_MASTER_ACK_LOW;
                        end else begin
                            sample_data[7:0] <= {read_shift[7:1], sda_in};
                            step <= 2'h0;
                            state <= S_MASTER_ACK_LOW;
                        end
                    end else begin
                ...
                S_STOP2: begin
                    scl_reg <= 1'b1;
                    sda_drive_low <= 1'b0;
                    // Publish both received bytes atomically. This prevents a
                    // software read from observing a torn temperature sample.

...
module lhr_ext_mem_bridge(
    input wire clk,
    input wire rst,

    input wire req_i,
    input wire we_i,
    input wire[`LHR_MemAddrBus] addr_i,
    input wire[`LHR_MemBus] wdata_i,
    input wire[3:0] byteen_i,
    output reg[`LHR_MemBus] rdata_o,
    output reg ready_o,
    output reg busy_o,
    output reg error_o,
    ...
    reg[4:0] state;
    reg cmd;
    reg[`LHR_MemAddrBus] req_addr;
    reg[`LHR_MemBus] req_wdata;
    reg[3:0] req_byteen;
    reg[7:0] req_checksum;
    reg[7:0] resp_status;
    reg[`LHR_MemBus] resp_rdata;
    reg[7:0] resp_checksum;

    wire[7:0] cmd_byte = (cmd == `LHR_WriteEnable) ? CMD_WRITE : CMD_READ;
    ...
    always @ (posedge clk) begin
        if (rst == `LHR_RstEnable) begin
            state <= S_IDLE;
            cmd <= `LHR_WriteDisable;
            req_addr <= `LHR_ZeroWord;
            req_wdata <= `LHR_ZeroWord;
            req_byteen <= 4'hf;
            req_checksum <= 8'h0;
            resp_status <= STATUS_OK;
            resp_rdata <= `LHR_ZeroWord;
            resp_checksum <= 8'h0;
            rdata_o <= `LHR_ZeroWord;
            ready_o <= `LHR_False;
            busy_o <= `LHR_False;
            error_o <= `LHR_False;
            ext_data_o <= 8'h00;

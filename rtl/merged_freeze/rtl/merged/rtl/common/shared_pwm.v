/*
 * Shared four-channel PWM peripheral for the merged TinyRISC-V design.
 * Reset is active low.
 */

// All cores keep their original slave address, while one selected request
// reaches this single physical PWM instance.
module shared_pwm(
    input wire clk,
    input wire rst,
    input wire we_i,
    ...
    assign pwm_o[0] = enable[0] && (period[0] != 32'h0) &&
                      (counter[0] < high_time[0]);
    assign pwm_o[1] = enable[1] && (period[1] != 32'h0) &&
                      (counter[1] < high_time[1]);
    assign pwm_o[2] = enable[2] && (period[2] != 32'h0) &&
                      (counter[2] < high_time[2]);
    assign pwm_o[3] = enable[3] && (period[3] != 32'h0) &&
                      (counter[3] < high_time[3]);

    always @ (posedge clk) begin
        if (rst == 1'b0) begin
            enable <= 4'h0;
            for (i = 0; i < 4; i = i + 1) begin
                period[i] <= 32'h0000_0000;
                high_time[i] <= 32'h0000_0000;
                counter[i] <= 32'h0000_0000;
            end
        end else begin
            if (we_i == 1'b1) begin
                case (addr_i[23:16])
